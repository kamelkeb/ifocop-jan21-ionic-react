...
const initialState = {
  ...
  prénom: "Martin",
  solde: 500,
  nombreEntréeAdécouvert: 0,
};

export const soldeReducer = (currentState, action) => {
  switch (action.type) {
    case "retrait":
      const entréeEnDécouvert =
        currentState.solde >= 0 && currentState.solde - action.payload < 0;
      const nouveauNombreEntréeAdécouvert =
        currentState.nombreEntréeAdécouvert + (entréeEnDécouvert ? 1 : 0);
      return {
        ...currentState,
...
export const useSoldeFeature = () => {
  ...
  const déposerDesSous = (x) => {
    ...
    dispatch({ type: "dépot", payload: Number(x) });
  };
  const retirerDesSous = (x) => {
    if (Number.isNaN(Number(x))) {
      throw new Error("Fais pas le malin!");
    }
    dispatch({ type: "retrait", payload: Number(x) });
  };
